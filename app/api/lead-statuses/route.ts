...
export const GET = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const statuses = await LeadStatus.find({ workspaceId, isActive: true }).sort({ order: 1, name: 1 });
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const { name, color, description, order, isDefault } = validationResult.data;
    ...
    if (finalOrder === undefined) {
      const lastStatus = await LeadStatus.findOne({ workspaceId }).sort({ order: -1 });
