...
export const DELETE = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (!workspaceId || !statusId) {
      return NextResponse.json({ message: 'Workspace ID and Status ID are required' }, { status: 400 });
    }
    ...
    if (status.isDefault) {
      return NextResponse.json({ message: 'Cannot delete default status' }, { status: 400 });
    }
