...
const updateContactSchema = z.object({
  ...
  accountManager: z.string().regex(/^[a-f\d]{24}$/i, 'Invalid user ID format').optional().or(z.literal('')),
  ...
  milestones: z.array(z.object({
    ...
    type: z.enum(['payment', 'meeting', 'contract', 'delivery', 'other']).optional(),
...
export const PUT = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (processedUpdateData.lastContactDate) {
      processedUpdateData.lastContactDate = new Date(processedUpdateData.lastContactDate).toISOString();
    }
    if (processedUpdateData.nextFollowUpDate) {
      processedUpdateData.nextFollowUpDate = new Date(processedUpdateData.nextFollowUpDate).toISOString();
    }

    // Handle milestones date conversion
    if (processedUpdateData.milestones) {
      processedUpdateData.milestones = processedUpdateData.milestones.map(milestone => ({
        ...milestone,
...
export const DELETE = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (!workspaceId || !contactId) {
      return NextResponse.json({ message: 'Workspace ID and Contact ID are required' }, { status: 400 });
    }
