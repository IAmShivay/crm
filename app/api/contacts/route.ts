...
const createContactSchema = z.object({
  ...
  originalLeadId: z.string().regex(/^[a-f\d]{24}$/i, 'Invalid lead ID format').optional(),
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const contactData = {
      ...
      lastContactDate: processedData.lastContactDate ? new Date(processedData.lastContactDate) : undefined,
      nextFollowUpDate: processedData.nextFollowUpDate ? new Date(processedData.nextFollowUpDate) : undefined,
      leadConversionDate: processedData.convertedFromLead ? new Date() : undefined
    };
    ...
    try {
      ...
      console.error('Failed to log contact creation activity:', activityError);
    }
