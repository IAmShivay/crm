...
export const DELETE = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (!workspaceId || !roleId) {
      return NextResponse.json({ message: 'Workspace ID and Role ID are required' }, { status: 400 });
    }
    ...
    if (role.isDefault) {
      return NextResponse.json({ message: 'Cannot delete default role' }, { status: 400 });
    }
