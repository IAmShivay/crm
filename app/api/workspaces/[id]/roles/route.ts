...
export const GET = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (!userPermissions.includes('roles.view') && !['Owner', 'Admin'].includes(membership.roleId?.name)) {
      return NextResponse.json(
    ...
    log.info(`Roles retrieved for workspace ${workspaceId} by user ${userId}`, {
      workspaceId,
      userId,
      roleCount: roles.length,
      duration: Date.now() - startTime
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    // Check permissions
    const userPermissions = membership.roleId?.permissions || [];
    if (!userPermissions.includes('roles.create') && !['Owner', 'Admin'].includes(membership.roleId?.name)) {
      return NextResponse.json(
        { message: 'Access denied. Insufficient permissions to create roles.' },
        { status: 403 }
      );
    }
