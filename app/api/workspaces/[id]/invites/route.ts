...
export const GET = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (!userPermissions.includes('members.view') && !['Owner', 'Admin'].includes(membership.roleId?.name)) {
      return NextResponse.json(
        { message: 'Access denied. Insufficient permissions to view invitations.' },
        { status: 403 }
      );
    }
    ...
    log.info(`Invitations retrieved for workspace ${workspaceId} by user ${userId}`, {
      workspaceId,
      userId,
      inviteCount: invitations.length,
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest, { params }: { params: { id: string } }) => {
  const startTime = Date.now();
  ...
  try {
    ...
    // Check permissions
    const userPermissions = membership.roleId?.permissions || [];
    if (!userPermissions.includes('members.invite') && !['Owner', 'Admin'].includes(membership.roleId?.name)) {
      return NextResponse.json(
    ...
    log.info(`Member invited to workspace ${workspaceId} by user ${userId}`, {
      workspaceId,
      userId,
      invitedEmail: email,
      roleId,
