...
import { Workspace, WorkspaceMember, Role, User, Activity } from '@/lib/mongodb/client';
...
const createWorkspaceSchema = z.object({
  ...
  settings: z.object({
    dateFormat: z.enum(['MM/DD/YYYY', 'DD/MM/YYYY', 'YYYY-MM-DD', 'DD-MM-YYYY', 'MM-DD-YYYY']).default('MM/DD/YYYY'),
    timeFormat: z.enum(['12h', '24h']).default('12h'),
    weekStartsOn: z.number().min(0).max(6).default(0),
    language: z.enum(['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh']).default('en')
  }).optional()
});
  ...
  try {
    ...
    try {
      memberships = await WorkspaceMember.find({
        userId,
        status: 'active'
      }).populate('workspaceId').populate('roleId');
    } catch (populateError) {
      ...
      for (const membership of rawMemberships) {
        try {
          ...
          console.error(`Error fetching workspace/role for membership ${membership._id}:`, fetchError);
        }
      }
    ...
    for (const membership of memberships) {
      ...
      if (!membership.workspaceId || typeof membership.workspaceId === 'string') {
        console.log('Found membership with unpopulated workspaceId:', membership._id);
      ...
      if (!membership.workspaceId.name) {
        console.log('Found membership with workspace missing name:', membership.workspaceId._id);
        orphanedMembershipIds.push(membership._id);
        continue;
      }
    ...
    if (orphanedMembershipIds.length > 0) {
      console.log(`Cleaning up ${orphanedMembershipIds.length} orphaned memberships in GET`);
      WorkspaceMember.deleteMany({ _id: { $in: orphanedMembershipIds } })
        .catch(cleanupError => console.error('Error cleaning up orphaned memberships in GET:', cleanupError));
    }

    // Map valid workspaces
    const workspaces = validMemberships
      .map(membership => ({
        ...
        role: membership.roleId && typeof membership.roleId === 'object'
          ? {
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const { name, description, currency, timezone, settings } = validation.data;
    ...
    for (const membership of existingMemberships) {
      ...
      if (!membership.workspaceId.name) {
        console.log('Found membership with workspace missing name:', membership.workspaceId._id || membership.workspaceId);
        orphanedMembershipIds.push(membership._id);
        continue;
      }
    ...
    const hasWorkspaceWithName = validMemberships.some(membership => {
      ...
      console.log(`Comparing workspace name "${workspaceName}" with input "${inputName}"`);
    ...
    try {
        ...
        try {
          ...
          if (roleError.code === 11000) {
            ...
            if (!ownerRole) {
              ...
              await ownerRole.save();
              console.log(`Created owner role with unique name: ${uniqueName}`);
            } else {
      ...
      try {
        ...
        console.error('Failed to log workspace creation activity:', activityError);
