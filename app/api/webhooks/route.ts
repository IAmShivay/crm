...
export const GET = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const webhooks = await Webhook.find({ workspaceId }).sort({ createdAt: -1 });
...
export const POST = withSecurityLogging(withLogging(async (request: NextRequest) => {
  ...
  try {
    ...
    const { workspaceId, name, description, webhookType, events, headers, transformationRules, retryConfig } = validationResult.data;
