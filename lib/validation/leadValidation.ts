...
export const leadFormSchema = yup.object({
  ...
  source: yup
    .string()
    .oneOf(['manual', 'website', 'referral', 'social', 'social_media', 'email', 'phone', 'other'], 'Invalid source')
    .default('manual'),
  ...
  assignedTo: yup
    .string()
    .nullable()
    .transform((value) => value === '' || value === 'unassigned' ? null : value),
...
export const customFieldSchema = yup.object({
  key: yup
    ...
    .matches(/^[a-zA-Z0-9_-]+$/, 'Field name can only contain letters, numbers, underscores, and hyphens')
    .trim(),
...
export const validateCustomFields = (customFields: Record<string, any>): string[] => {
  const errors: string[] = [];
