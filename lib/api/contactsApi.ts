...
export const contactsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getContacts: builder.query<ContactsListResponse, ContactsQueryParams>({
      query: ({ workspaceId, page = 1, limit = 20, search, status, category, assignedTo, priority }) => {
    ...
    getContact: builder.query<ContactResponse, { id: string; workspaceId: string }>({
      query: ({ id, workspaceId }) => `/${id}?workspaceId=${workspaceId}`,
      providesTags: (result, error, { id }) => [{ type: 'Contact', id }],
    }),

    // Create a new contact
    createContact: builder.mutation<ContactResponse, { data: CreateContactRequest; workspaceId: string }>({
    ...
    // Update a contact
    updateContact: builder.mutation<ContactResponse, { id: string; data: UpdateContactRequest; workspaceId: string }>({
    ...
    // Delete a contact
    deleteContact: builder.mutation<{ success: boolean; message: string }, { id: string; workspaceId: string }>({
    ...
    convertLeadToContact: builder.mutation<ConvertLeadToContactResponse, { leadId: string; workspaceId: string }>({
