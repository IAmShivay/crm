...
const LeadActivitySchema = new Schema<ILeadActivity>({
  ...
  activityType: {
    type: String,
    enum: ['created', 'updated', 'status_changed', 'assigned', 'note_added', 'converted', 'deleted'],
    required: true
  },
  performedBy: {
    type: String,
...
if (typeof window === 'undefined') {
  ...
  LeadActivitySchema.index({ createdAt: 1 }, { expireAfterSeconds: 2 * 365 * 24 * 60 * 60 });
}

export const LeadActivity = mongoose.models?.LeadActivity || mongoose.model<ILeadActivity>('LeadActivity', LeadActivitySchema);
