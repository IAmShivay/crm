...
export class MongoDBClient {
  ...
  async getUserWorkspaces(userId: string): Promise<IWorkspace[]> {
    await this.ensureConnection();
    const members = await WorkspaceMember.find({ userId, status: 'active' }).populate('workspaceId');
    return members.map((member: any) => member.workspaceId as IWorkspace);
  }

  // Lead operations
  async getLeads(workspaceId: string, filters?: { status?: string }): Promise<ILead[]> {
    await this.ensureConnection();
  ...
  async getActivities(workspaceId: string, limit: number = 50): Promise<IActivity[]> {
    await this.ensureConnection();
