...
export async function rateLimit(
  ...
  const config = { ...RATE_LIMIT_CONFIGS[endpoint] || RATE_LIMIT_CONFIGS.default, ...customConfig };
  ...
  if (rateLimitData.blocked && rateLimitData.blockedUntil && now < rateLimitData.blockedUntil) {
  ...
  if (rateLimitData.requests.length >= config.maxRequests) {
    ...
    log.security('Rate limit exceeded - potential DDoS attempt', {
      identifier,
      endpoint,
 ...
 */
export function createRateLimiter(endpoint: string, customConfig?: Partial<RateLimitConfig>) {
...
export class AdvancedRateLimiter {
  private tokenBuckets = new Map<string, {
    tokens: number;
    lastRefill: number;
    capacity: number;
    refillRate: number;
