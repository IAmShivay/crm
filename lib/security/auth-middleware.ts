...
export async function requireWorkspaceAuth(
  ...
  workspaceId: string,
  requiredPermission?: string
): Promise<{ success: true; user: any; workspace: any } | { success: false; response: NextResponse }> {
...
export function withWorkspaceAuth(
  handler: (request: NextRequest, context: { user: any; workspace: any; params: any }) => Promise<NextResponse>,
  requiredPermission?: string
) {
  return async (request: NextRequest, context: { params: any }) => {
    const workspaceId = context.params?.workspaceId || request.nextUrl.searchParams.get('workspaceId');
    ...
    const authResult = await requireWorkspaceAuth(request, workspaceId, requiredPermission);
...
export function recordFailedLogin(identifier: string) {
  ...
  if (attempts.count >= 5) {
    ...
    logSecurityEvent('brute_force_detected', { identifier, attempts: attempts.count }, 'high');
  }
