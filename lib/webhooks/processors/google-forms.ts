...
export class GoogleFormsProcessor implements WebhookProcessor {
  ...
  async process(data: any, request: NextRequest): Promise<ProcessedWebhookData> {
    ...
    try {
      ...
      throw new Error(`Failed to process Google Forms webhook: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private processDirectFormat(data: any): ProcessedLead | null {
    try {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (!['name', 'full_name', 'email', 'email_address', 'phone', 'phone_number', 
              'company', 'company_name', 'organization', 'message', 'comments', 
              'additional_info', 'budget', 'estimated_budget', 'timestamp'].includes(key)) {
