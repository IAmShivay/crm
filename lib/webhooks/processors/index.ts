...
export function detectWebhookType(request: NextRequest, data: any): string {
  ...
  // Check for LinkedIn
  if (userAgent.includes('linkedin') || data.leadGenForms || data.sponsoredAccount) {
    return 'linkedin';
  }
 ...
 */
export function getProcessorInfo(type: string): { name: string; description: string } {
  const processor = WEBHOOK_PROCESSORS[type.toLowerCase()];
  if (!processor) {
  ...
  const descriptions: Record<string, string> = {
    facebook: 'Facebook Lead Ads - Processes leads from Facebook advertising campaigns',
    'google-forms': 'Google Forms - Processes form submissions from Google Forms',
    linkedin: 'LinkedIn Lead Gen Forms - Processes leads from LinkedIn advertising',
    ...
    generic: 'Generic - Processes leads from any source with flexible field mapping',
  };
