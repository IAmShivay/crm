...
export class SwipePagesProcessor implements WebhookProcessor {
  ...
  validate(data: any): boolean {
    ...
    return data && (data.email || data.name || data.first_name || data.last_name);
  }
  ...
  async process(data: any, request: NextRequest): Promise<ProcessedWebhookData> {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      const mappedField = this.FIELD_MAPPINGS[key.toLowerCase() as keyof typeof this.FIELD_MAPPINGS];
    ...
    const noteFields = ['message', 'comments', 'notes', 'additional_info', 'description'];
    ...
    if (data.form_name) {
      tags.push(`form-${String(data.form_name).toLowerCase().replace(/\s+/g, '-')}`);
    }
    
    // Add campaign tag if available
    if (data.utm_campaign) {
      tags.push(`campaign-${String(data.utm_campaign).toLowerCase().replace(/\s+/g, '-')}`);
    }
